...
export class AppSyncGraphQLAPIStack extends Stack {
  ...
  constructor(scope: Construct, id: string, props: StackProps, context: CDKContext) {
    ...
    for (const lambdaDefinition of lambdaDefinitions) {
      ...
      if (lambdaDefinition.name === 'get-books') {
        const lambdaDS = graphqlApi.addLambdaDataSource('getBooksDS', lambdaFunction);
        lambdaDS.createResolver({ typeName: 'Query', fieldName: 'getBooks' });
      }
      if (lambdaDefinition.name === 'add-book') {
        const lambdaDS = graphqlApi.addLambdaDataSource('addBookDS', lambdaFunction);
        lambdaDS.createResolver({ typeName: 'Mutation', fieldName: 'addBook' });
      }
      if (lambdaDefinition.name === 'update-book') {
        const lambdaDS = graphqlApi.addLambdaDataSource('updateBookDS', lambdaFunction);
        lambdaDS.createResolver({ typeName: 'Mutation', fieldName: 'updateBook' });
      }
      if (lambdaDefinition.name === 'delete-book') {
        const lambdaDS = graphqlApi.addLambdaDataSource('deleteBookDS', lambdaFunction);
        lambdaDS.createResolver({ typeName: 'Mutation', fieldName: 'deleteBook' });
      }
